...
{if(Serv[i].cpu >0)
{tot_cpu += ((Node_CPU-Serv[i].cpu)/Node_CPU*(Node_CPU-Serv[i].cpu)/Node_CPU);
}
  if(Serv[i].mem >0)
{
tot_mem += ((Node_CPU-Serv[i].cpu)/Node_CPU*(Node_CPU-Serv[i].cpu)/Node_CPU);
}
}
for(i=0;i<chrom->length;i++) 
{
...
chrom->fitness = (tot_mem+tot_cpu)/no_of_nodes;
