...
#define MAXVMS 200 
#define MAXSER 200   /* The maximum number of tasks */ 
struct{ 
	int cpu; 
} VM[MAXVMS]; 
struct{ 
...
{if(Serv[i].cpu > 0)
{tot_cpu_waste +=(Node_CPU-Serv[i].cpu)/Node_CPU;}
}
for(i=0;i<chrom->length;i++) 
{
vm_id[i] = vm_index[i];
...
if(tot_cpu_waste>0)
{fit_fun = no_of_nodes*(1/tot_cpu_waste);}
else
{fit_fun = no_of_nodes;}
